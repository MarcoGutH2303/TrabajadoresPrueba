...
GO
ALTER PROC spListarTrabajadores
AS
BEGIN
	SELECT	T0.Id,
			T0.TipoDocumento,
			...
			T0.Sexo,
			T0.IdDepartamento,
			T1.NombreDepartamento,
			T0.IdProvincia,
			T2.NombreProvincia,
			T0.IdDistrito,
			T3.NombreDistrito
	FROM Trabajadores T0
	INNER JOIN Departamento T1 ON T0.IdDepartamento=T1.Id
	INNER JOIN Provincia T2 ON T0.IdProvincia=T2.Id AND T0.IdDepartamento=T2.IdDepartamento
	INNER JOIN Distrito T3 ON T0.IdDistrito=T3.Id AND T0.IdProvincia=T3.IdProvincia 
	ORDER BY T0.Id;
END
GO
